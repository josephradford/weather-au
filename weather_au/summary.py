...
class Summary:
    ...
    def summary(self):
        ...
        result = collections.OrderedDict(
            ...
            temp_later=Item(
                fd_curr["now"]["later_label"], fd_curr["now"]["temp_later"], "°"
            ),
            temp_feels_like=Item(
                "Feels Like", self.observations["temp_feels_like"], "°"
            ),
            chance_of_rain=Item(
                "Chance of any Rain", self.forecasts_daily[0]["rain"]["chance"], "%"
            ),
        )

        # TODO: at 11:30pm I say 0-2mm, 20% and website says 20%
        # which is the next three hourly value???
        rain_amount = fd_curr["rain"]["amount"]

        if rain_amount["max"] is not None and rain_amount["max"] > 0:
            result["possible_rainfall"] = Item(
                "Possible Rainfall",
                f"{rain_amount['min']}-{rain_amount['max']}",
                rain_amount["units"],
            )
    ...
    def today(self):
        ...
        return {"extended_text": fd_curr["extended_text"]}
