...
class Place:
    ...
    def __init__(self, state=None, location=None):
        ...
        req = urllib.request.Request(
            self.url, data=None, headers={"User-Agent": weather_au.PLACES_USER_AGENT}
        )
    ...
    def air_temperature(self):
        ...
        if temp_node is None:
            raise PlaceException(
                f"Could not parse HTML ({self.url}, tag=li class=airT)"
            )
    ...
    def forecast(self):
        ...
        if forecasts_top is None:
            raise PlaceException(
                f"Could not parse HTML ({self.url}, tag=div class=forecasts-top)"
            )
        ...
        if "issued at" not in issued_text:
            raise PlaceException(
                f"Could not parse HTML ({self.url}, span missing 'issued at')"
            )
        ...
        if forecast_summary is None:
            raise PlaceException(
                f"Could not parse HTML ({self.url}, tag=dl class=forecast-summary)"
            )
        ...
        if precis_node is None:
            raise PlaceException(
                f"Could not parse HTML ({self.url}, tag=dd class=summary)"
            )
    ...
    def station_id(self):
        ...
        if station_p is None:
            raise PlaceException(
                f"Could not parse HTML ({self.url}, tag=p class=station_id)"
            )
